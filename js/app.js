...
$(document).ready(function () {
    ...
    $(".btn-question").on('click', function (e) {
        ...
        if (answer.length > 1) {
            ...
            new_question += ' <p> ' + determineQuestion(question, answer, username) + '</p> </div>';
...
function determineQuestion(old_question, answer, username) {
    ...
    if (new_question) {
        return new_question.B && username.length ? username + new_question.B + getCountry() + '?' : new_question.B;
    }
    return 'Ok';
}
...
function isAnswerTheSame(answer) {
    if (typeof origin !== 'undefined') {
        return  answer == origin.country ? dataset[4] : dataset[3];
    }
    return dataset[3];
}
